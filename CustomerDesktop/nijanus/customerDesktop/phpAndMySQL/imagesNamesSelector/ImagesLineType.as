...
{
	// Класс типа строки изображений.
	public class ImagesLineType
	{
		...
		public static const KNOWN_VALUE_MINIMUM_INDEX =
			ImagesLineType.VALUES.indexOf( ImagesLineType.UPPER, 0 );
		// Максимальный индекс известного значения.
		public static const KNOWN_VALUE_MAXIMUM_INDEX =
			ImagesLineType.VALUES.indexOf( ImagesLineType.LOWER, 0 );	
		//-----------------------------------------------------------------------
		// Статические методы.
		...
		public static function ValueIsExist( parValue: String ): Boolean
		{
			...
			if ( ImagesLineType.VALUES.indexOf( parValue, 0 ) < 0 )
				// Признак ложен.
				return false;
		...
		public static function IndexIsOfKnownValue( parValueIndex: int ): Boolean
		{
			...
			if
			(
				( parValueIndex < ImagesLineType.KNOWN_VALUE_MINIMUM_INDEX ) ||
				( parValueIndex > ImagesLineType.KNOWN_VALUE_MAXIMUM_INDEX )
			)
				// Признак ложен.
				return false;
		...
		public static function IndexOfValue( parValue: String ): int
		{
			...
			return ImagesLineType.VALUES.indexOf( parValue, 0 );
		} // IndexOfValue		
		...
		public static function ValueOfIndex( parValueIndex: int ): String
		{
			...
			(
				...
				return ImagesLineType.UNKNOWN;
			// Если индекс находится в пределах индексов известных значений.
			else
				// Значение по индексу из массива.
				return ImagesLineType.VALUES[ parValueIndex ];
		} // ValueOfIndex		
...
} // nnijanus.customerDesktop.phpAndMySQL.imagesNamesSelector