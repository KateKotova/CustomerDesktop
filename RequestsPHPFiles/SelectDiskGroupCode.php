<?php
	// Скрипт, возвращающий код группы диска.
	...
	$parNomenclaturesGroupsCodesColumnName =
		$_GET[ 'NomenclaturesGroupsCodesColumnName' ];
	...
	$mySQLQueryString =
		"SELECT {$parNomenclaturesGroupsCodesColumnName} " .
...
*/
	...
	while ( $mySQLQueryFetch = mysql_fetch_assoc( $mySQLQueryID ) )
	{
		...
		$xmlTree .= "\t<Row>\n";
		// Элемент XML-дерева - столбец кодов групп в таблице номенклатур.
		$xmlTree .=
			"\t\t<{$parNomenclaturesGroupsCodesColumnName}>" .
			( int ) $mySQLQueryFetch[ $parNomenclaturesGroupsCodesColumnName ] .
			"</{$parNomenclaturesGroupsCodesColumnName}>\n";
